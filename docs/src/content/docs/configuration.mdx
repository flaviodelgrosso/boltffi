...
The default output is `dist/java`. This produces Java sources, a C header, JNI glue, a structured native output under `dist/java/native/<host-target>/`, and a flat current-host `_jni` compatibility copy in `dist/java/`. `host_targets` defaults to `["current"]`; in Phase 3 it may only resolve to the current machine. Requires `JAVA_HOME` and `clang`.
