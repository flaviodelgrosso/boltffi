...
`host_targets` defaults to `["current"]`. In Phase 3, `boltffi pack java` only supports packaging the current host target. `boltffi pack java --no-build` is intentionally unsupported; rerun without `--no-build`.
